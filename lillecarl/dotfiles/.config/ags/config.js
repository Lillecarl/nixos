...
import { lookUpIcon, exec, execAsync } from 'resource:///com/github/Aylur/ags/utils.js';
...
const Clients = () => Widget.Box({
    ...
    children: Hyprland.bind('clients').transform(clients => {
        return clients.map(({ address, title, class: iconn }) => Widget.Button({
            //on_clicked: () => Hyprland.sendMessage(`dispatch client ${id}`),
            child: Widget.Icon({ icon: iconn }),
            class_name: address,
        }));
    }),
});
...
const ClientIcon = () => Widget.Icon({
    icon: Hyprland.active.client.bind('class').transform(name => {
      if (lookUpIcon(name)) {
        return name
      }
...
const TBar = (monitor = 0) => Widget.Window({
...
const LBar = (monitor = 0) => Widget.Window({
    name: `lbar-${monitor}`, // name has to be unique
    class_name: 'bar',
    monitor,
