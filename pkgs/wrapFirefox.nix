  ...
  wrapper =
    ...
    stdenv.mkDerivation {
      ...
      buildCommand = ''
        ...
        pdir=$(pwd)
        omniPatches=(${builtins.toString omniPatches})
        if test -n "$omniPatches"; then
          export omniTmp=$out/lib/${libName}/browser/omniTmp
          mkdir -p $omniTmp
          ${lib.getExe unzip} -oq $out/lib/${libName}/browser/omni.ja -d $omniTmp || true
          unlink $out/lib/${libName}/browser/omni.ja
          cd $omniTmp
