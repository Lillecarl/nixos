...
git = local["git"]
nix = local["nix"]
nix_prefecth_github_latest_release = local["nix-prefetch-github-latest-release"]
...
def updategit():
  ...
  for versionfile in local.cwd // "**/version.json":
    ...
    newdata = json.loads(nix_prefecth_github_latest_release("--json", versiondata["owner"], versiondata["repo"]))
    ...
    try:
      ...
      repo = github.get_repo("{0}/{1}".format(versiondata["owner"], versiondata["repo"]))
